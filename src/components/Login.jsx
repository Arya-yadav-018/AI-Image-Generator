...
const Login = () => {
    ...
    const {setShowLogin , backendurl , setToken , setUser ,laodCreditsData } = useContext(AppContext)
    ...
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
       ...
       <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
         ...
         { state === 'Login' ?  <p className='mt-5 text-center'>Don't have an account ? <span className='text-blue-600 cursor-pointer' onClick={()=>setState('Sign up')}>Sign up</span> </p>
          ...
          <img onClick={()=>setShowLogin(false)} src={assets.cross_icon} className='absolute top-5 right-5 cursor-pointer p-1 transition-all duration-300 hover:rounded-full hover:border hover:border-gray-400r'/>
